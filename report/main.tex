...
\begin{abstract}
    ...
    The application of entropy regularization reduced variance and improved policy gradient approaches by encouraging action space exploration.
    Meanwhile, bootstrapping with n-step: 3 had no effect on the final results, and combining bootstrapping and baseline subtraction had no noticeable effect.
...
\begin{algorithm}[htbp]
    ...
    \label{alg:reinforce}
    \begin{algorithmic}
        \STATE {\bfseries Input:} $env, \pi_{\theta}, \alpha, \gamma$
        ...
        \REPEAT
            ...
            \ENDFOR
        \UNTIL{convergence}
        \STATE {\bfseries Output:} $\pi_{\theta}$
    \end{algorithmic}
\end{algorithm}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{figs/sweep.png}
...
\begin{algorithm}[htbp]
    ...
    \label{alg:ac}
    \begin{algorithmic}
        \STATE {\bfseries Input:} $env, \pi_{\theta}, V_{\phi}, \alpha_{\pi}, \alpha_{V}, \gamma$
